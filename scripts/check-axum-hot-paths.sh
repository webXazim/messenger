...
require realtime/src/commands.rs "pub(crate) async fn conversation_event_audiences" "Durable chat events do not resolve participant user audiences"
require realtime/src/commands.rs "kind: AudienceKind::User" "Message events cannot reach participants whose conversation is closed"
require realtime/src/message_interactions.rs "conversation_event_audiences(tx, conversation_id)" "Receipt and reaction events cannot reach background participant devices"
require realtime/src/message_mutations.rs "conversation_event_audiences(tx, conversation_id)" "Message edit and delete events cannot reach background participant devices"
require realtime/src/call_commands.rs "conversation_event_audiences(tx, conversation_id)" "Call events cannot reach background participant devices"
...
require realtime/src/message_interactions.rs "COALESCE(target.sequence, 0)" "Axum receipt pointers are not advanced by durable message sequence"
...
require realtime/src/presence.rs "LAST_SEEN_PERSIST_INTERVAL_SECONDS" "Axum last-seen persistence is not write-rate limited"
require realtime/src/call_commands.rs "deleted_text_backup,deletion_source,client_temp_id" "Axum call timeline inserts do not satisfy the current message schema"
