(function () {
  ...
  function renderKnowledge(body) {
    ...
    if (state.selectedArticle) {
      ...
      var article = node("article", "cs-kb-article"); article.appendChild(node("small", "", state.selectedArticle.category ? state.selectedArticle.category.name : "Support article")); article.appendChild(node("h2", "", state.selectedArticle.title)); var articleContent = node("div", "cs-kb-article-content"); articleContent.innerHTML = state.selectedArticle.body || ""; article.appendChild(articleContent);
    ...
    (state.knowledge.articles || []).forEach(function (article) { var item = node("button", "cs-kb-item"); item.type = "button"; item.appendChild(node("strong", "", article.title)); item.appendChild(node("span", "", article.summary || (function () { var content = document.createElement("div"); content.innerHTML = String(article.body || ""); return String(content.textContent || "").replace(/\s+/g, " ").trim().slice(0, 120); })())); item.onclick = function () { loadKnowledgeArticle(article.id).catch(function () {}); }; list.appendChild(item); }); section.appendChild(list); body.appendChild(section);
  }
