...
function StatusComposer({ onClose, onPublished }: { onClose: () => void; onPublished: (status: UserStatus) => void }) {
  ...
  const error = publishMutation.error ? parseApiError(publishMutation.error, "Status could not be shared.").message : "";
  ...
  return createPortal(
    <div className="ms-status-modal" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !publishMutation.isPending) onClose(); }}>
      <section className="ms-status-composer" role="dialog" aria-modal="true" aria-labelledby="status-composer-title">
        <header>
          <div><span>24-hour update</span><h2 id="status-composer-title">Create status</h2></div>
          <button type="button" className="ms-status-icon-button" onClick={onClose} disabled={publishMutation.isPending} aria-label="Close status composer"><CloseIcon /></button>
        </header>

        <div className="ms-status-composer__modes" role="tablist" aria-label="Status type">
        ...
        {mode === "media" && file ? (
          ...
          <div className="ms-status-composer__text-preview" style={{ backgroundColor, color: "#ffffff" }}>
            <textarea autoFocus value={text} maxLength={800} onChange={(event) => setText(event.target.value)} placeholder="What's on your mind?" aria-label="Status text" />
          </div>
        )}
        ...
        <footer>
          ...
          <button type="button" className="ms-button ms-button--primary" disabled={!canPublish} onClick={() => publishMutation.mutate()}>{publishMutation.isPending ? "Sharing…" : "Share status"}</button>
        </footer>
      </section>
    </div>,
    document.body,
...
function StatusViewer({ groups, groupId, initialIndex, currentUserId, onClose, onViewed, onDeleted, onMessage }: {
  ...
  return createPortal(
    <div className="ms-status-viewer" role="dialog" aria-modal="true" aria-label={`${personDisplayName(group.author)}'s status`}>
      ...
      <div className="ms-status-viewer__stage" style={status.content_type === "text" ? { backgroundColor: status.background_color, color: status.text_color } : undefined}>
        {status.content_type === "image" && status.media?.preview_url ? <img src={status.media.preview_url} alt={status.text || `${personDisplayName(group.author)}'s status`} /> : null}
      ...
      <header className="ms-status-viewer__header">
        ...
        <div className="ms-status-viewer__identity">
          ...
          <strong>{isOwn ? "Your status" : personDisplayName(group.author)}</strong>
          <span>{timeAgo(status.created_at)}</span>
        </div>
        <div className="ms-status-viewer__actions">
          {status.content_type === "video" ? <button type="button" onClick={() => setMuted((value) => !value)} aria-label={muted ? "Unmute status" : "Mute status"}><SoundIcon muted={muted} /></button> : null}
          <button type="button" onClick={onClose} aria-label="Close status"><CloseIcon /></button>
        </div>
      </header>
...
export function StatusTray({ currentUser, friends = [], busyUserId, onOpenFriend }: {
  ...
  return (
    <>
      <section className="ms-status-tray" aria-label="Status updates">
        <div className="ms-status-tray__heading"><strong>Updates</strong>{statusesQuery.isFetching && !statuses.length ? <span>Loading…</span> : <span>24h</span>}</div>
        <div className="ms-status-tray__scroll ms-scroll-region" role="list">
          <div className="ms-status-tray__item ms-status-tray__item--own" role="listitem">
            <button type="button" className={`ms-status-tray__person${ownGroup ? " has-status" : ""}`} onClick={openOwn} aria-label={ownGroup ? "View your status" : "Add your status"}>
              <span className="ms-status-tray__ring"><UserAvatar person={currentUser} size="md" decorative /></span>
              <span>{ownGroup ? "Your status" : "Add status"}</span>
            </button>
            <button type="button" className="ms-status-tray__add" onClick={() => setComposerOpen(true)} aria-label="Create a new status"><PlusIcon /></button>
          </div>
